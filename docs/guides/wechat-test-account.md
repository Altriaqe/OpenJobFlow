...
首次正式个人订阅号文章使用最近完整的 `2026-08-27` 四关键词快照生成。接口返回 `generated`、`new_job_count=247`、`baseline_ready=true`，六个文章包文件齐全。

导入 Markdown 后完成以下真实检查：

- 247 条岗位原始地址保留，旧 Markdown 外部超链接为 0；
- 草稿保存后岗位名称、薪资、公司、地点、技能和地址仍存在；
- 手机预览显示封面、标题、趋势图、正文和结尾免责声明；
- 用户最终确认正式发布成功。

运行日期不一定等于快照日期。若当天定时抓取尚未完成，生成当天文章会返回 `409`；应使用最近完整快照，不能把不存在的数据日期伪装成当日数据。

## V1.3.4 Windows 一键拉取

V1.3.4 将人工 SSH、打包、SCP、解压和中文文件名修正收敛为本地 CMD/PowerShell 工具。不同电脑使用自己的环境变量或命令参数，真实用户名、服务器地址、路径和密码不写入公共仓库。工具只下载并校验文章包，不触发采集、Telegram 或公众号发布。

完整使用方式见 [`wechat-article-download.md`](wechat-article-download.md)。
