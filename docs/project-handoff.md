...
09:14 定时任务完成四城抓取（各 45 条，共 180 条）并完成 ETL；Telegram 图文日报状态为 `sent`，微信文章包状态为 `generated`。正式微信公众号草稿创建返回 `failed`，错误码为 `wechat_draft_failed`，因此每日服务以退出码 1 正确结束。该失败未回滚 ETL、文章包或 Telegram，且不得因本次失败重复发送 Telegram。

截图仅显示草稿状态接口的通用错误码，尚不足以判断微信 API、素材、账号权限、IP 白名单或网络路径中的具体根因。下一步只读查询当日草稿状态的脱敏 `error_message`，并先检查公众号后台是否已存在同日草稿；确认不存在后，才评估是否对单日草稿进行受控重试。

随后已确认数据库/API 状态为 `failed` 且 `has_draft=false`，执行一次单独微信草稿创建仍返回同样的 `wechat_draft_failed`。服务器拉取 `57d9104` 后，首次构建因清华 PyPI 镜像 SSL 提前断开失败；临时改用官方 PyPI 构建成功，API 重建并通过健康检查。之后单独创建微信草稿返回 `status=created`、`has_draft=true`，未重复发送 Telegram。维护者最终确认今日 Telegram 与微信公众号双链路正常。原始失败的具体外部原因仍未追溯，但 V1.3.7 的安全阶段错误码已部署，后续失败可区分 Token、封面、正文图和草稿请求阶段。

## 2026-09-20 当前状态

今日任务因 BOSS 登录状态失效而停止。维护者通过虚拟桌面重新登录并完成单次恢复运行，随后确认 Telegram 消息与微信公众号草稿均正常到达；本次仍属于人工恢复样本。

当前公开文档状态整理为 V1.3.7 稳定性与可观测性维护版：微信草稿创建失败会让每日服务明确失败，并通过安全阶段错误码区分 Token、封面、正文图和草稿请求阶段；不会回滚已经完成的 ETL 或 Telegram。项目仍不宣称能够自动恢复过期登录态，也不宣称具备高可用能力。公开 README 已同步中英文说明，Python 包版本仍为 `0.1.0`。

## 2026-09-18 当前状态

维护者确认今日 Telegram 消息与微信公众号草稿均正常到达。本次结果增加一个双链路正常运行样本，不单独作为长期稳定性结论。
...
今日定时链路完成，维护者确认 Telegram 消息与微信公众号草稿均正常到达。微信草稿失败状态修复 `cf7fb00` 已部署；本次结果记为一个正常运行样本，不代表外部网络、BOSS 登录态和定时任务已经长期稳定。
...
今天的 systemd timer 已按时触发，但每日任务在 BOSS 登录/CDP `Page.navigate` 超时检查阶段安全停止。VNC 恢复登录并通过 `--check` 后，微信草稿创建成功；Telegram 文字和图片投递结果为 `text_uncertain`，维护者确认手机端未收到。

已新增并部署受保护的 `/reports/daily/multi/recover` 恢复接口，提交为 `40fc9c8`。在 `confirm_not_received=true` 下，该接口先重发文字，取得明确成功后才发送图片；文字再次不确定时停止。2026-09-13 已通过该流程确认 Telegram 文字和图片补发成功。普通发送接口语义未改变。

本地服务测试为 `33 passed`，Ruff、语法编译和 `git diff --check` 通过；API 测试因本机缺少 `openai` 依赖未能收集。当前双链路恢复是人工确认后的恢复证据，不等于 BOSS 登录失效、CDP 超时和网络异常已经自动修复，后续继续观察 timer。
