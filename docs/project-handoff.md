...
随后已确认公众号状态为 `failed` 且 `has_draft=false`，执行一次单独微信草稿创建仍返回同样的 `wechat_draft_failed`。本地已增加安全的阶段错误码（Token、封面、正文图、草稿请求），但尚未部署到服务器；服务器需拉取新代码、重新构建并重建 API 后，下一次受控运行才能定位具体阶段。
