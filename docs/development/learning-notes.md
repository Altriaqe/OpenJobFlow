...
2026-08-28 凌晨生成当天文章时接口返回 `409`，原因是 09:00 定时抓取尚未产生 `2026-08-28` 完整快照。改用最近完整的 `2026-08-27` 快照后，接口返回 `generated`、`new_job_count=247`、`baseline_ready=true`。这说明“操作日期”和“数据快照日期”必须分开，发布时不能假设当天数据已经存在。

真实文章包包含机器 Markdown、动态中文导入 Markdown、HTML、封面、趋势图和清单。服务器运行目录由容器创建，普通用户可以读取但不能在其父目录新建压缩包，因此压缩包写到用户可写目录后再通过 SSH/SCP 下载。Windows 解压 tar 时中文文件名出现乱码，但正文 UTF-8 完整；只重命名唯一的公众号导入 Markdown 后继续使用。

正式公众号后台完成草稿保存、手机预览和最终发布，247 条明文岗位原始地址与结尾免责声明均保留。当前发布链路为“服务器生成文章包 → 维护者下载 → 公众号人工审核与发布”，不是公众号 API 全自动发布。下一步只优化一键下载和解压，不跳过人工发布确认。

## V1.3.4 Windows 一键拉取公众号文章包

2026-08-28 定时任务成功生成 256 个新增岗位，Telegram 返回 `sent`，微信文章状态为 `generated`。新增 `scripts/download-wechat-article.ps1`：默认使用当天日期，也可指定日期；通过一次递归 SCP 拉到本地临时目录，校验五个固定文件和唯一导入 Markdown，再检查 `manifest.json.report_date` 与 `new_job_count`，最后移动到 `downloads/wechat-日期/wechat`。目标目录存在时在联网前停止，避免覆盖用户已经审核的文章。

Windows PowerShell 5 会把无 BOM 的 UTF-8 中文脚本按系统编码解析，真实测试曾在中文 `Write-Host` 行报字符串未终止。脚本和测试文件因此保存为 UTF-8 BOM，并用 Windows 自带 `powershell.exe` 复测。另提供 `download-wechat-article.cmd`，让 CMD 用户或双击运行时无需处理 PowerShell 换行和执行策略。

真实验收已从 Ubuntu 拉取 2026-08-28 六文件文章包，正确显示新增岗位 256、中文导入文件名、建议标题和作者。脚本不触发采集、不发送 Telegram、不填写公众号后台字段，也不自动发布；人工导入、预览和发布边界保持不变。
