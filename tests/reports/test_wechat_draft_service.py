...
def test_create_draft_uploads_images_and_records(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("jobflow.reports.wechat_draft_service.create_draft", capture_draft)
    monkeypatch.setattr(
        "jobflow.reports.wechat_draft_service.record_wechat_draft_created",
    ...
    content = captured["articles"][0]["content"]
    assert "trend.png" not in content
    assert "https://img" in content
