...
**V1.3.7 是稳定性与可观测性维护版。** 当前公开基线包含分层 ETL 流水线、只读分析 API、Telegram 推送和可选的微信公众号待审核草稿创建。Telegram 与微信保持独立失败边界；微信草稿创建失败时，每日服务现在会明确失败，不再在草稿未完成时报告整体成功。

最近一次运行观察中，上游 BOSS 登录态失效，维护者通过虚拟桌面重新登录后恢复运行，Telegram 与微信链路均正常。这只是一次人工恢复样本，不代表高可用；自托管部署在上游登录态失效时仍需要人工恢复浏览器登录。

这里的项目版本指公开应用与文档基线，不修改 `pyproject.toml` 中的 Python 包版本。
...
AI 层不直接连接 PostgreSQL，也不能执行任意 SQL；它只接收固定应用查询返回的结构化结果。消息渠道不参与采集、标准化或数据库写入。Telegram 与微信分支独立：V1.3.6 保留 V1.3.5 的正式公众号草稿流程，并让草稿失败明确反映到每日服务状态；维护者仍需在后台审核和手动发布，Windows 下载工具保留为人工兜底。
...
V1.3.6 增加了微信草稿失败的明确状态。V1.3.7 进一步增加安全的阶段错误码，用于区分 Token 获取、封面上传、正文图片上传和草稿请求失败。项目不会回滚已经完成的 ETL 或 Telegram 结果，也不宣称能够自动恢复过期的上游登录态或具备高可用能力。
