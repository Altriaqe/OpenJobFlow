...
**V1.3.6 是稳定性与可观测性维护版。** 当前公开基线包含分层 ETL 流水线、只读分析 API、Telegram 推送和可选的微信公众号待审核草稿创建。Telegram 与微信保持独立失败边界；微信草稿创建失败时，每日服务现在会明确失败，不再在草稿未完成时报告整体成功。
...
V1.3.6 是稳定性与可观测性维护版：微信草稿失败会让每日工作流明确失败，但不会回滚已经完成的 ETL 或 Telegram 结果。项目不宣称能够自动恢复过期的上游登录态，也不宣称具备高可用能力。
